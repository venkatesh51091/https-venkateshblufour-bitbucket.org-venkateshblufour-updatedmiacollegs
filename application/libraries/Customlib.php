...
class Customlib {
  ...
  public function send_mail_to($toemail, $subject, $body) {
        $this->CI->load->model('emailconfig_model');
        $emailconf = $this->CI->emailconfig_model->getActiveEmail();        
        $mail = new PHPMailer();
        $mail->IsSMTP(); // we are going to use SMTP
        $mail->SMTPAuth = true; // enabled SMTP authentication
        $mail->SMTPSecure = $emailconf->ssl_tls;  // prefix for secure protocol to connect to the server
        $mail->Host = $emailconf->smtp_server;      // setting GMail as our SMTP server
        $mail->Port = $emailconf->smtp_port;                   // SMTP port to connect to GMail
        $mail->Username = $emailconf->smtp_username;  // user email address
        $mail->Password = $emailconf->smtp_password;            // password in GMail
        $mail->SetFrom($emailconf->smtp_username, 'Smart School');  //Who is sending the email
        $mail->AddReplyTo($emailconf->smtp_username, $emailconf->smtp_username);  //email address that receives the response
        $mail->Subject = $subject;
        $mail->Body = $body;
        $mail->AltBody = $body;
        $mail->AddAddress($toemail); // Who is addressed the email to
        if ($mail->Send()) {
            return TRUE;
        } else {
            return FALSE;
