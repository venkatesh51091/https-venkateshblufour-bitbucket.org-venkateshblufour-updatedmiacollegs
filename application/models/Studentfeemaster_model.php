...
class Studentfeemaster_model extends CI_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null) {
        $sql = "SELECT IFNULL(`student_fees_master`.`id`, '0') as `student_fees_master_id`,`classes`.`id` AS `class_id`,"
                ...
                . "WHERE `student_session`.`session_id` =  " . $this->current_session . " AND `student_session`.`class_id` = " . $this->db->escape($class_id)
                . " AND `student_session`.`section_id` = " . $this->db->escape($section_id) . "  ORDER BY `students`.`id`";
    ...
    public function fee_deposit($data, $send_to) {
        ...
        if ($q->num_rows() > 0) {
            ...
            $result = $this->smsgateway->sentAddFeeSMS($row->id, $inv_no, $send_to);
            return $row->id;
        } else {
            ...
            $result = $this->smsgateway->sentAddFeeSMS($inserted_id, 1, $send_to);
            return $inserted_id;
        }
    }
